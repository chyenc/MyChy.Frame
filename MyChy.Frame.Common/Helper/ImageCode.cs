...
namespace MyChy.Frame.Common.Helper
{
    public sealed class ImageCode
    {
        ...
        public static void SessionCode(string sessionName, int num)
        {
        ...
        {
            var imgcode = StringHelper.GenerateRandomCode(num);
            CreatImage(imgcode);
        ...
        private static void CreatImage(string checkCode)
        {
            ...
            try
            {
                ...
                for (var i = 0; i < 25; i++)
                {
                    var x1 = rnd.Next(img.Width);
                    var x2 = rnd.Next(img.Width);
                    var y1 = rnd.Next(img.Height);
                    var y2 = rnd.Next(img.Height);

                    g.DrawLine(new Pen(Color.Silver), x1, y1, x2, y2);
                }


                var fontFamilyArray = new[] { "Tahoma", "Arial", "Times New Roman" };
                ...
                for (var i = 0; i < checkCode.Length; i++)
                {
                    ...
                    g.DrawString(checkCode[i].ToString(), font, new SolidBrush(RandomColor(seed + i + 1)), x, y);
                    x += fontsize + 3;
                }
                //g.Flush();

                var tImg = TwistImage(img, RandomColor(seed));
        ...
        private static Bitmap TwistImage(Bitmap srcBmp, Color linecolor)
        {
            ...
            for (var i = 0; i < srcBmp.Width; i++)
            {
                for (var j = 0; j < srcBmp.Height; j++)
                {
                    ...
                    if (color == Color.FromArgb(0)) continue;
                    float k = 0;
                    if (j <= hh)
                    ...
                    var dx = k * (float)maxDx * (float)rn / 100;
                    var x = i + dx;
