...
using MyChy.Frame.Common.EfData.Extensions;
using MyChy.Frame.Common.EfData.Helper;

namespace MyChy.Frame.Common.EfData
{
    /// <summary>
    ...
    public abstract class EFRepositoryBase<TEntity, TKey> : IRepository<TEntity, TKey> where TEntity : EntityBase<TKey>
    {
        ...
        /// <summary>
        /// 获取 当前实体的查询数据集(通过读上下文进行读取，只读专用，返回的实体数据不会被上下文跟踪)
        /// </summary>
        public IQueryable<TEntity> ReadEntities
        {
            get
            {
                return EFContext.Set<TEntity, TKey>().AsNoTracking();
            }
        }
        ...
        /// <returns>操作影响的行数</returns>
        public virtual int Update(Expression<Func<TEntity, object>> propertyExpression, TEntity entity, bool isSave = true)
        {
